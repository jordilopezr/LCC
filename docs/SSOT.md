...
*   **IAP Tunnel nativo:** ✅ **Enviado en 27H1** — módulo `native/src/iap_tunnel/` (frame/error/session/listener) implementa el protocolo de túnel IAP directamente en Rust, sin depender del binario `gcloud`. `tunnel.rs` prioriza el motor nativo con fallback automático a `gcloud` si el path nativo falla; la variable de entorno `LCC_TUNNEL_ENGINE` (`native`/`gcloud`) fuerza un motor concreto para depuración. `TunnelFailure` es el primer módulo de la app que usa códigos de error tipados propagados por el bridge (9 claves `tunnelError*` localizadas EN/ES); `doctor.rs`, `sftp.rs`, `snapshots.rs` y `gcloud.rs` siguen pendientes de migrar a errores tipados. La relación con la VM real (sesión SSH/RDP/SFTP sostenida, recuperación tras corte de red) no se puede ejercitar en tests automatizados — ver checklist de QA manual en `docs/superpowers/plans/2026-07-20-native-iap-tunnel-manual-qa.md`.
